...
export function render(data = {}, pageConfig = {}, brandConfig = { name: 'Siluet', domain: 'siluet.web.id' }) {
    const title = data.title || 'Amplop Digital Cashless';
    const subtitle = data.subtitle || 'Bagi keluarga dan sahabat yang ingin memberikan tanda kasih secara digital:';
    ...
    const styleId = `digital-gift-sage-styles-${Math.random().toString(36).substr(2, 9)}`;

    const accountsHtml = bankAccounts.map((acc, idx) => `
        <div class="${theme.cardBg} border border-emerald-500/30 rounded-3xl p-6 text-center flex flex-col items-center justify-between shadow-xl relative z-10 transition-all hover:-translate-y-1">
            <div class="w-12 h-12 rounded-full bg-emerald-500/15 text-emerald-300 border border-emerald-500/30 flex items-center justify-center text-xl font-bold mb-4">
                💳
            </div>
            <span class="text-xs font-bold uppercase tracking-wider text-emerald-300 mb-1">${acc.bank_name || 'Bank'}</span>
            ...
            <button
                ...
                class="bg-emerald-600/20 hover:bg-emerald-600/30 text-emerald-200 border border-emerald-500/30 w-full py-2 px-4 rounded-xl text-xs font-bold transition-all cursor-pointer shadow-sm active:scale-95"
            >
                📋 Salin No. Rekening
            </button>
        </div>
    ...
    return `
        ...
        <section id="digital_gift" class="py-20 md:py-28 px-6 ${sectionBgClass} relative overflow-hidden">
            ...
            <div class="max-w-5xl mx-auto text-center relative z-10">
                ...
                ${qrisUrl ? `
                    <div class="${theme.cardBg} border border-emerald-500/30 rounded-3xl p-6 max-w-sm mx-auto mb-12 text-center shadow-xl">
                        <span class="text-xs font-bold text-emerald-300 uppercase tracking-wider block mb-3">Scan QRIS All Payment</span>
                        <img src="${qrisUrl}" alt="QRIS Digital Gift" class="w-48 h-48 object-contain rounded-2xl mx-auto border border-emerald-700 bg-white p-2 shadow-inner" />
                    </div>
                ` : ''}

                ${rsvpWhatsapp ? `
                    <div class="pt-6 border-t border-emerald-800/40 max-w-xl mx-auto">
                        <p class="${theme.subtitle} text-xs mb-4">Ingin mengonfirmasi kehadiran atau mengirim ucapan doa restu?</p>
                        <a href="https://wa.me/${rsvpWhatsapp.replace(/[^0-9]/g, '')}?text=Halo,%20saya%20ingin%20mengonfirmasi%20kehadiran%20di%20acara%20pernikahan..." target="_blank" rel="noopener noreferrer" class="bg-emerald-600 hover:bg-emerald-500 text-white px-8 py-3 rounded-full text-xs font-black inline-flex items-center gap-2 shadow-lg hover:scale-105 transition-all">
