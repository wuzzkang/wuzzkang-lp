...
export function render(data = {}, pageConfig = {}, brandConfig = { name: 'Siluet', domain: 'siluet.web.id' }) {
    ...
    const groomIg = data.groom_instagram || 'romeo_adiputra';
    ...
    const brideIg = data.bride_instagram || 'juliet_saraswati';
    ...
    const styleId = `wedding-couple-styles-${Math.random().toString(36).substr(2, 9)}`;
        ...
        <section id="wedding_couple" class="py-20 md:py-28 px-6 ${sectionBgClass} relative overflow-hidden">
            ...
            <div class="max-w-5xl mx-auto text-center relative z-10">
                ...
                <div class="mb-14 max-w-2xl mx-auto">
                    <span class="text-2xl font-serif text-amber-400 block mb-3">بِسْمِ ٱللَّهِ ٱلرَّحْمَٰنِ ٱلرَّحِيمِ</span>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-14 items-center">
                    <!-- Groom Profile -->
                    <div class="bg-slate-900/70 border border-amber-500/30 rounded-3xl p-8 text-center flex flex-col items-center shadow-xl transition-all hover:-translate-y-1">
                        <div class="mb-6 relative">
                            <img src="${groomPhoto}" alt="${groomName}" class="w-32 h-32 md:w-40 md:h-40 rounded-full object-cover border-4 border-amber-400/40 shadow-xl mx-auto ring-4 ring-amber-500/10" />
                        </div>
                        <h3 class="wedding-font-serif text-xl md:text-2xl font-extrabold text-white mb-1 tracking-tight">${groomName}</h3>
                        <span class="text-xs font-bold text-amber-400 uppercase tracking-wider mb-3">Mempelai Pria (${groomNickname})</span>
                        <p class="text-slate-400 text-xs md:text-sm leading-relaxed max-w-xs mb-4">${groomParents}</p>
                        ${groomIg ? `
                            <a href="https://instagram.com/${groomIg.replace('@', '')}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-amber-500/10 text-amber-400 border border-amber-500/20 text-xs font-bold hover:bg-amber-500/20 transition-all">
                    ...
                    <div class="bg-slate-900/70 border border-amber-500/30 rounded-3xl p-8 text-center flex flex-col items-center shadow-xl transition-all hover:-translate-y-1">
                        <div class="mb-6 relative">
                            <img src="${bridePhoto}" alt="${brideName}" class="w-32 h-32 md:w-40 md:h-40 rounded-full object-cover border-4 border-amber-400/40 shadow-xl mx-auto ring-4 ring-amber-500/10" />
                        </div>
                        <h3 class="wedding-font-serif text-xl md:text-2xl font-extrabold text-white mb-1 tracking-tight">${brideName}</h3>
                        <span class="text-xs font-bold text-amber-400 uppercase tracking-wider mb-3">Mempelai Wanita (${brideNickname})</span>
                        <p class="text-slate-400 text-xs md:text-sm leading-relaxed max-w-xs mb-4">${brideParents}</p>
                        ${brideIg ? `
                            <a href="https://instagram.com/${brideIg.replace('@', '')}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-amber-500/10 text-amber-400 border border-amber-500/20 text-xs font-bold hover:bg-amber-500/20 transition-all">
