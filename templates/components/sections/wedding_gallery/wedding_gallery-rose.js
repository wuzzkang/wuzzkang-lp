...
export function render(data = {}, pageConfig = {}, brandConfig = { name: 'Siluet', domain: 'siluet.web.id' }) {
    const title = data.title || 'Galeri Moments of Love';
    const subtitle = data.subtitle || 'Abadikan keceriaan dan canda tawa kami bersama';
    ...
    const styleId = `wedding-gallery-rose-styles-${Math.random().toString(36).substr(2, 9)}`;
    ...
    return `
        ...
        <section id="wedding_gallery" class="py-16 md:py-24 px-6 ${sectionBgClass} relative overflow-hidden">
            ...
            <div class="max-w-5xl mx-auto text-center relative z-10">
                <div class="mb-12">
                    <span class="text-xs font-bold text-rose-300 uppercase tracking-widest block mb-2 px-4 py-1 rounded-full bg-rose-500/15 border border-rose-500/30 w-max mx-auto">🌸 PREWEDDING GALLERY</span>
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ${images.map((img, idx) => `
                        <div class="group relative overflow-hidden rounded-2xl md:rounded-3xl border border-rose-500/30 aspect-square shadow-2xl cursor-pointer">
                            ...
                            <div class="absolute inset-0 bg-gradient-to-t from-rose-950/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-4">
                                <span class="text-rose-200 text-xs font-bold">Momen #${idx + 1}</span>
                            </div>
                        </div>
                    `).join('')}
                </div>
